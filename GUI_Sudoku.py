...
class SudokuApp:
    ...
    def draw(self):
        ...
        if self.selected:
            self.draw_selection_box(self.selected)
...
############ Helper Functions ##############
    ...
    def draw_selection_box(self, pos):
        pygame.draw.rect(self.window, LIGHT_BLUE,
                         (GRID_POSITION[0] + (pos[1] * CUBE_SIZE), GRID_POSITION[1] + (pos[0] * CUBE_SIZE), CUBE_SIZE, CUBE_SIZE))

    def draw_grid(self):
        pygame.draw.rect(self.window, BLACK, (GRID_POSITION[0], GRID_POSITION[1], GRID_WIDTH, GRID_HEIGHT), 3)
        for i in range(1, 9):
            pygame.draw.line(self.window, BLACK, (GRID_POSITION[0], GRID_POSITION[1] + (
                i * CUBE_SIZE)), (GRID_POSITION[0] + GRID_WIDTH, GRID_POSITION[1] + (i * CUBE_SIZE)), 3 if i % 3 == 0 else 1)
            pygame.draw.line(self.window, BLACK, (GRID_POSITION[0] + (i * CUBE_SIZE), GRID_POSITION[1]), (GRID_POSITION[0] + (
                i * CUBE_SIZE), GRID_POSITION[1] + GRID_HEIGHT), 3 if i % 3 == 0 else 1)

    def draw_locked_cubes(self, locked):
        for cube in locked:
            pygame.draw.rect(self.window, GREY, (cube[1]*CUBE_SIZE+GRID_POSITION[0],
                                                 cube[0]*CUBE_SIZE+GRID_POSITION[1], CUBE_SIZE, CUBE_SIZE))

    def draw_numbers(self, window):
        if self.temp:
            self.text_to_screen(window, self.temp, [self.selected[1]*CUBE_SIZE +
                                                    GRID_POSITION[0], self.selected[0]*CUBE_SIZE+GRID_POSITION[1]])
        for row in range(9):
            for col in range(9):
    ...
    def extra_func(self, window):
        ...
        window.blit(timer_text, (WIDTH-GRID_POSITION[0]-timer_font_width,
                                 GRID_POSITION[1]-timer_font_height - GRID_POSITION[0]))
